...
import sys
from main import main, command_create_dict, command_load_dict, command_solve_countdown
...
class TestMain(unittest.TestCase):
	...
	@patch('main.command_solve_countdown')
	@patch('main.command_load_dict')
	def test_main_solve_countdown(self, mock_command_load_dict, mock_command_solve_countdown, mock_input):
		...
		This test simulates user input to select the option to solve the countdown problem
		and then exit the program. It verifies that the appropriate functions are called
		and the expected output is produced.

		Args:
			mock_command_solve_countdown (MagicMock): Mocked command_solve_countdown function.
			mock_input (MagicMock): Mocked input function.
	...
	@patch('main.solve_countdown')
	@patch('main.output_words')
	def test_command_solve_countdown_success(self, mock_output_words, mock_solve_countdown, mock_input):
		...
		This test simulates user input for the letters and mocks the solve_countdown and
		output_words functions. It verifies that the appropriate functions are called
		and the expected output is produced.
		...
		Asserts:
			...
			The output contains the message "Countdown problem solved successfully".
		"""
		mock_dict = {'test': 'dictionary'}
		mock_solve_countdown.return_value = [{'word': 'abc', 'definition': 'test'}]
		
		with patch('sys.stdout', new=StringIO()) as fake_out:
			command_solve_countdown(mock_dict)
		
		mock_solve_countdown.assert_called_once_with('abcdefghi', mock_dict)
		mock_output_words.assert_called_once_with([{'word': 'abc', 'definition': 'test'}])
		self.assertIn("Countdown problem solved successfully", fake_out.getvalue())


	@patch('builtins.input', return_value='INVALID')
	def test_command_solve_countdown_invalid_input(self, mock_input):
		"""
		Test the command_solve_countdown function when invalid input is provided.

		This test simulates user input with an invalid number of letters. It verifies
