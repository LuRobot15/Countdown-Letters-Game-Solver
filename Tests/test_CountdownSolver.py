...
class TestCheckAnswer(unittest.TestCase):
    """
    This class contains tests for the check answer function
    in CountdownSolver the module
    """
    ...
    def test_check_valid_word(self):
        """
        checks that the function works properlyt for a valid word
        only returning the definitions of the given word.
        All of them.
        """
        results_dict = check_answer("apple", "apples", self.sample_dict)

        self.assertTrue(results_dict["correct"])
        self.assertEqual(len(results_dict["definitions"]), 2)
